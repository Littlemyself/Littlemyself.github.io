<img src="https://qn.ymfgm.com/wp-content/uploads/2021/tu/12222.jpg" alt="442" />
...
在系统变量里双击选择path,选择新建，将FFmpeg的bin目录的路径D:\ffmpeg\bin;加进去，记得加上分号，点击“确定”保存，即配置完成。（我放的是系统变量的Path里）
...
```
自己常用的参数：-filter:v fps=fps=15  帧数15   -compression_level 1  压缩等级：1  -q:v 5  压缩率：5%  -loop 0  循环播放
```shell
ffmpeg -i input.mp4 -vcodec libwebp -filter:v fps=fps=15 -lossless 0 -compression_level 1 -q:v 5 -loop 0 -preset default -an -vsync 0 output.webp
```
...
- 设置预设渲染模式 -preset default , 可按视频画面内容类型设置 picture, photo, text, icon, drawing 或 none。选择合适的渲染模式可导出更小的 webp 文件。 [ffmpeg.org](https://link.zhihu.com/?target=http%3A//ffmpeg.org/ffmpeg-all.html%23Options-28)
...
<img class="aligncenter" src="https://qn.ymfgm.com/wp-content/uploads/2020/12/20210206021744.webp" alt="dgd" style="zoom: 200%;" />